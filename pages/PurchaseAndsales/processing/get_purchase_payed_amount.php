...
if ($_POST['supplierId'] != NULL && $_POST['paymentDate'] !=NULL) {
    ...
    $id= $_POST['supplierId'];
    $sqlSelect = "SELECT SUM(ppList.`amount_paid`) AS `amount_paid` FROM `purchase_payment_list` AS ppList INNER JOIN purchase_payment AS pp ON pp.purchase_payment_id = ppList.`purchase_payment_id` WHERE pp.supplier_id= '$id'";
