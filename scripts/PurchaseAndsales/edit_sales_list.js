    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        if (remainingQuantity != 'nothing') {
          ...
          let editQuantity = +$(`#quantity${id}`).val();
          $(`#remainingQuantity${id}`).val(+remainingQuantity+editQuantity);
        } else {
...
(function editDisplay() {
    ...
    $.ajax({
        ...
        url: "../../../pages/PurchaseAndsales/transaction/edit_sales_list_ajax.php",
        success: function (response) {
            var editList = JSON.parse(response);
            $('#lrNumber').val(editList[0].lr_number);
            let editSalesLength = editList.length;
            for (let i = 0; i < editSalesLength; i++) {
                addNewRow();
